...
const backend = {
  localAPI: async function(action, json = {}, api_url = null) {
  ...
  toWei: function toWei(eth, decimals) {
    return new BigNumber(String(eth))
      .times(new BigNumber(10 ** decimals))
      .floor()
      .toString();
  },
  toEth: function toEth(wei, decimals) {
  ...
  returnSignature: async function returnSignature(msgToSignIn, privateKeyIn) {
    const privateKey =
      privateKeyIn.substring(0, 2) === "0x"
        ? privateKeyIn.substring(2, privateKeyIn.length)
        : privateKeyIn;
    const salted = ethereumjs.hashPersonalMessage(
      ethereumjs.toBuffer(msgToSignIn)
    );
    const sig = mapValues(
      ethereumjs.ecsign(salted, new Buffer(privateKey, "hex")),
      (value, key) => (key === "v" ? value : ethereumjs.bufferToHex(value))
    );
    return sig;
  },
  ...
  returnOrderBookForUser: async function returnOrderBookForUser(address) {
    return await this.api(
      `returnOrderBookForUser`,
      { address },
      "https://api-regional.idex.market/"
    );
  },
  /**
   * order
